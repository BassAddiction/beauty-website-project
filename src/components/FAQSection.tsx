...
const FAQSection = () => {
  ...
  const faqs: FAQ[] = [
    ...
    },
    {
      ...
      answer: "Да, наш VPN обеспечивает полный доступ к YouTube, заблокированным соцсетям, ChatGPT, Netflix и другим заблокированным сервисам с высокой скоростью."
    },
    {
      question: "Продлил подписку, а в VPN-клиенте ошибка подключения!",
      answer: "Вы продлили подписку. Username остался прежним, но UUID меняется. Вам нужно добавить новую ссылку в VPN-клиент, полученную при оплате — она приходит на почту, а также в личный кабинет. Если у вас есть Happ или V2rayTun, добавить новую ссылку можно одним кликом из личного кабинета. Помните: Username — это ваше всё, не передавайте его никому. Почему так сделано? Это сделано в целях безопасности, потому что у нас нет ограничений по количеству устройств. У нас есть одно ограничение — 30 ГБ трафика в сутки. Например, вы купили подписку на 1 месяц, подключили себя, маму и своего бойфренда. Прошел месяц, вы расстались со второй половинкой, продлили наш сервис, ваша подписка обновилась, вы обновили себе и маме, а бойфренд остался в прошлом. Он больше не имеет доступа к VPN за ваш счёт. Вот так примерно это работает."
    }
  ];
